...
def read():
    with open ('TelephoneDirectory.csv', 'r', encoding = 'utf-8') as my_f:
...
def search():
    with open('TelephoneDirectory.csv', 'r', encoding = 'utf-8') as my_f1:
...
def add():
    with open('TelephoneDirectory.csv', 'a', encoding = 'utf-8') as my_f2:
        line = input('Введите через пробел Имя Фамилия Номер_телефона\n')
        my_f2.write(line + '\n')
    return line

def change():
    with open('TelephoneDirectory.csv', 'r', encoding = 'utf-8') as my_f3:
        tel_dir = my_f3.readlines()
        flag = 1
        name = input('Номер телефона записи, которую хотите изменить\n')
        with open('TelephoneDirectory.csv', 'w', encoding = 'utf-8') as my_f:
            for line in tel_dir:       
                if name in line:
                    flag = 0
                    print(line)
